...
Dialog.create("Quantify!");
	...
	Dialog.addCheckbox("Candida/deconvolved", false); // the C. albicans images I quantified were obtained from decolvolution of z-stacks from a wide-field microscope. Different setting are therefore needed in this case
    Dialog.show();
...
// the shortened loop can be used for testing
			...
			run("Clear Results");
			selectWindow(list[i]);
		if (Ca == true) wait(WC); else wait(W);	
			roiManager("Select", j);
		if (Ca == true) wait(WC); else wait(W);	
